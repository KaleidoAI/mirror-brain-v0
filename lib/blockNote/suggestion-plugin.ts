...
export type SuggestionMenuReturn<
  T extends SuggestionItem
> = {
...
export class SuggestionPluginManager<T extends SuggestionItem> extends EventEmitter<any> {
  ...
  constructor(isEditable: boolean) {
    ...
    this.selectEventName = "selectItem";
    this.updateEventName = "update";
  }

  public invokeOnUpdate(state: SuggestionsMenuState<T>) {
    this.emit(this.updateEventName, state);
  }

  public registerOnUpdate(
    callback: (state: SuggestionsMenuState<T>) => void
  ) {
    return this.on(this.updateEventName, callback);
  }

  public invokeOnSelectItem(
    queryRangeFrom: number,
    item: T,
  ) {
    this.emit(this.selectEventName, queryRangeFrom, item);
  }

  public registerOnSelectItem(
    callback: (
      queryRangeFrom: number,
      item: T,
    ) => void
  ) {
    return this.on(this.selectEventName, callback);
  }
...
> {
  ...
  constructor(
    private readonly suggestionPluginManager: SuggestionPluginManager<T>,
    ...
    this.updateSuggestionsMenu = () => {
      ...
      suggestionPluginManager.invokeOnUpdate(this.suggestionsMenuState);
    };
  ...
  update(view: EditorView, prevState: EditorState) {
    ...
    if (stopped || !this.suggestionPluginManager.isEditable) {
    ...
    if (this.suggestionPluginManager.isEditable) {
...
>(
...
) : SuggestionMenuReturn<T> => {
  // Assertions
  if (defaultTriggerCharacter.length !== 1) {
    throw new Error("'char' should be a single character");
  }

  let suggestionsPluginView: SuggestionsMenuView<T>;
  ...
  return {
    plugin: new Plugin({
      ...
      view: () => {
        suggestionsPluginView = new SuggestionsMenuView<T>(
      ...
      props: {
        handleKeyDown(view, event) {
          ...
          if (event.key === "Enter") {
            ...
            deactivate(view);
            suggestionPluginManager.invokeOnSelectItem(
